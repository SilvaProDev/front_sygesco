            ...
            <div class="col-lg-11 mb-4">
              ...
              <div class="card">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  <h6 class="m-0 font-weight-bold text-primary">La liste des rôles</h6>
                ...
                <div class="table-responsive">
                  <table class="table align-items-center table-flush" id="dataTable" width="100%" cellspacing="0">
                    ...
                    <tbody>
                      <tr v-for="(item, index) in gettersRole" :key="item.id">
                        <td>{{index+1}}</td>
                        <td>{{item.code}}</td>
                        <td>{{item.libelle}}</td>
                        <td v-if="item.permissions.length !=0">
                          <span v-for=" tem in item.permissions" :key="tem.id" class="badge badge-success">
                            {{tem.libelle}} 
                          </span>
                        ...
                        <td>
                           <button  @click="editModale(item.id)" title="Modifier" type="button"  data-toggle="modal" data-target="#exampleModalCenter">
                              ...
                              &nbsp;
                          <a title="Supprimer"  @click.prevent="SupprimerRole(item.id)">
  ...
  <div class="bloc-modale" v-if="showModal">
    <div   class="overlay">
      <div class="modale card" >
           ...
           <h4>Enrégistrement des rôles </h4>
           ...
           <div class="card-body">
             <form action="" method="post">
              ...
              <div class="form-group">
                  <label for="libelle">Rôle </label>   
                   <span v-if="verifcode">
                    <code style="color:red; font-style:italic;">Ce rôle existe déjâ</code>
                    </span>
                   <span style="color:red; font-style:italic;"
               ...
               <div class="form-group">
                      <label for="exampleFormControlSelect2">Liste des permissions</label>
                       <span style="color:red; font-style:italic;"
                            v-if="$v.formData.permissions.$error && !$v.formData.permissions.required"
                            role="alert"> 
                            <span style="color:red; font-weight:bold">*</span>
                            champs obligatoire!
                        </span> 
                      <select multiple class="form-control" id="exampleFormControlSelect2" 
                      @input="$v.formData.permissions.$touch()" v-model="formData.permissions">
                       <option v-for="item in gettersPermission" :key="item.id" :value="item.id"> {{item.libelle}} </option>
...
export default {
  ...
  computed:{
     ...mapGetters("parametres",["gettersNiveau","gettersTrimestre","gettersAnne", "gettersMatiere","","gettersClasse"]),
  ...
  methods:{
    ...mapActions("parametres",["getNiveau","ModifierNiveau","AjouterNiveau", "SupprimerNiveau","getAnnee","getClasse",
     ...
     ...mapActions("personnel",["getPermission", "getRole","AjouterRole","SupprimerRole",
                "ModifierRole"]),
    showModals:function(){
    ...
    //vider les champs
      ...
      editModale(id){
        ...
        this.editText = this.gettersRole.find(tem =>tem.id == id)
      },
    ...
    AjouterFonctionLocal(){
        ...
        this.AjouterRole(this.formData)
            this.$v.formData.$reset();
         this.formData.code="";
